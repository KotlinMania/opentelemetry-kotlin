# Claude Code Project Instructions — opentelemetry-kotlin

## Project Overview

This is **opentelemetry-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`opentelemetry`](https://crates.io/crates/opentelemetry).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/opentelemetry/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/opentelemetry>
