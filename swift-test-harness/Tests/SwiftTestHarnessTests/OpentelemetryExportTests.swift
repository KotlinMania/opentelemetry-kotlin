...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Opentelemetry.swiftmodule/`
...
//      `import Opentelemetry` above would fail at compile time.
//
//   2. The static archive `libOpentelemetry.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Opentelemetry" }` and
//      `flattenPackage = "io.github.kotlinmania.opentelemetry"` configuration in
...
//      via the `OpentelemetryLibrary` product.
//
...
final class OpentelemetryExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Opentelemetry swift module imported cleanly")
    }
}
